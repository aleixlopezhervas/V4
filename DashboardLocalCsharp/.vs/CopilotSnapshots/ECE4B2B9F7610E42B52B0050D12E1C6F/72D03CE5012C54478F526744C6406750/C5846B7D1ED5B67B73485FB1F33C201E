...
namespace Formulario
{
    public partial class Form1 : Form
    {
        ...
        private double destAlt = double.NaN; // Almacenar altitud destino
        ...
        private GMapOverlay flightWaypointsOverlay;
        private GMapOverlay flightDroneOverlay;
        ...
        private System.Windows.Forms.Timer waypointTimer;
        private const double WAYPOINT_ARRIVAL_THRESHOLD = 0.000005; // ~0.5 metros
        private const float FLIGHT_ALTITUDE = 5f;

        // Variables para vuelo desde base de datos
        ...
        private void navButton_Click(object sender, EventArgs e)
        {
            ...
            // Detener navegación automática si está activa
            if (tag == "Stop")
            {
        ...
        private void ProcesarTelemetria(byte id, List<(string nombre, float valor)> telemetria)
        {
            ...
            // Actualizar mapa con telemetría
        ...
        private void velocidadTrackBar_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (!double.IsNaN(destLat) && !double.IsNaN(destLon))
            {
                ...
                // Enviar comando al dron para ir al destino
        ...
        private void Alt_changeTrackBar_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            // Cambiar altitud si el método existe
        ...
        private void EnviarDronHacia(double lat, double lon, double alt)
        {
            ...
            // Usar la altitud actual del dron si no se especifica otra
            if (double.IsNaN(currentAlt))
            {
                destAlt = 5; // Altitud por defecto
            }
            else
            {
                destAlt = currentAlt; // Usar altitud actual del dron
            }

            // Intentar llamar al método goto usando reflexión
            try
            {
                var method = dron.GetType().GetMethod("goto");
                if (method != null)
                {
                    // Llamar con: goto(lat, lon, alt, blocking=false)
            ...
            // Si goto no existe, usar navegación automática con timer
            if (navigationTimer == null)
            {
                navigationTimer = new System.Windows.Forms.Timer();
                navigationTimer.Interval = 500; // Cada 500ms evaluar y ajustar dirección
        ...
        private void NavigationTimer_Tick(object sender, EventArgs e)
        {
            ...
            // Determinar la dirección de navegación basándose en dLat y dLon
        ...
        private string CalcularDireccion(double dLat, double dLon)
        {
            ...
            // Determinar la dirección de los 8 puntos cardinales
            // North: -22.5 a 22.5
            // NorthEast: 22.5 a 67.5
            // East: 67.5 a 112.5
            // SouthEast: 112.5 a 157.5
            // South: 157.5 a 202.5
            // SouthWest: 202.5 a 247.5
            // West: 247.5 a 292.5
            // NorthWest: 292.5 a 337.5
        ...
        private void ActualizarTelemetria(double lat, double lon, double alt, double heading)
        {
            ...
            // Actualizar mapa de vuelo en tiempo real cuando está activa la misión
            if (isFlying && flightGmap != null && !double.IsNaN(lat) && !double.IsNaN(lon))
            {
                var dronePos = new PointLatLng(lat, lon);

                // Actualizar o crear marcador del dron en el mapa de vuelo
                if (flightDroneMarker == null)
                {
                    flightDroneMarker = new RedDotMarker(dronePos, 8, heading);
                    flightDroneOverlay.Markers.Add(flightDroneMarker);
                }
                else
                {
                    flightDroneMarker.Position = dronePos;
                    ((RedDotMarker)flightDroneMarker).Heading = heading;
                }

                // Agregar punto al rastro si se ha movido una distancia significativa
                if (flightTrace.Count == 0 || 
                    Math.Abs(flightTrace[flightTrace.Count - 1].Lat - dronePos.Lat) > 0.000001 ||
                    Math.Abs(flightTrace[flightTrace.Count - 1].Lng - dronePos.Lng) > 0.000001)
                {
                    flightTrace.Add(dronePos);

                    // Actualizar el overlay del rastro de forma incremental
                    var traceMarker = new TraceMarker(dronePos);
                    flightTraceOverlay.Markers.Add(traceMarker);
                }

                // Refrescar mapa para mostrar cambios en tiempo real
                flightGmap.Refresh();
            }
        }
        ...
        // Envía la lista de objetos seleccionados
        ...
        private void Gmap_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                // Enviar comando al dron para ir a las coordenadas seleccionadas
        ...
        {
            flightGmap = new GMapControl();
            flightGmap.Dock = DockStyle.Fill;
            flightMapPanel.Controls.Add(flightGmap);

            GMaps.Instance.Mode = AccessMode.ServerAndCache;
            flightGmap.MapProvider = GMapProviders.GoogleSatelliteMap;
            flightGmap.Position = new PointLatLng(41.27, 1.98);
            flightGmap.MinZoom = 2;
            flightGmap.MaxZoom = 25;
            flightGmap.Zoom = 19;
            flightGmap.ShowTileGridLines = false;
            flightGmap.RetryLoadTile = 3;
            flightGmap.ShowCenter = false;
            flightGmap.IgnoreMarkerOnMouseWheel = true;

            flightTraceOverlay = new GMapOverlay("trace");
            flightGmap.Overlays.Add(flightTraceOverlay);

            flightWaypointLinesOverlay = new GMapOverlay("waypoint_lines");
            flightGmap.Overlays.Add(flightWaypointLinesOverlay);

            flightWaypointsOverlay = new GMapOverlay("waypoints");
            flightGmap.Overlays.Add(flightWaypointsOverlay);

            flightDroneOverlay = new GMapOverlay("drone");
            flightGmap.Overlays.Add(flightDroneOverlay);
        }
        ...
        private void loadWaypointsBtn_Click(object sender, EventArgs e)
        {
            ...
            openFileDialog.Title = "Cargar waypoints";
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    flightWaypointsOverlay.Markers.Clear();
                    flightWaypointLinesOverlay.Markers.Clear();
                    flightWaypointLinesOverlay.Routes.Clear();
                    flightTrace.Clear();
                    flightDroneOverlay.Markers.Clear();
                    ...
                    // Conectar waypoints con líneas azules finas
                    ...
                    waypointsLabel.Text = $"Waypoints ({waypoints.Count})";
                    flightGmap.Refresh();
        ...
        private void startFlightBtn_Click(object sender, EventArgs e)
        {
            if (waypoints.Count == 0)
            {
                MessageBox.Show("Por favor carga waypoints primero.");
                return;
            }
            ...
            startFlightBtn.Enabled = false;
            loadWaypointsBtn.Enabled = false;

            // Iniciar waypointTimer si no existe
            if (waypointTimer == null)
            {
                waypointTimer = new System.Windows.Forms.Timer();
                waypointTimer.Interval = 500; // Cada 500ms evaluar waypoint
                waypointTimer.Tick += WaypointTimer_Tick;
            }
        ...
        private void stopFlightBtn_Click(object sender, EventArgs e)
        {
            ...
            dron.Navegar("Stop");
            startFlightBtn.Enabled = true;
            loadWaypointsBtn.Enabled = true;
            currentWaypointIndex = 0;

            // Limpiar rastro y dron del mapa
            flightTrace.Clear();
            flightDroneOverlay.Markers.Clear();
            flightTraceOverlay.Markers.Clear();
            flightGmap.Refresh();

            Console.WriteLine("Vuelo detenido.");
        }

        private void WaypointTimer_Tick(object sender, EventArgs e)
        {
            if (!isFlying || currentWaypointIndex >= waypoints.Count)
            {
                isFlying = false;
                waypointTimer.Stop();
                startFlightBtn.Enabled = true;
                ...
                Console.WriteLine("Vuelo completado.");
                MessageBox.Show("Vuelo completado - Todos los waypoints alcanzados.");

                // Limpiar rastro DESPUÉS de que el usuario acepta el diálogo
                flightTrace.Clear();
                flightDroneOverlay.Markers.Clear();
                flightTraceOverlay.Markers.Clear();
                flightGmap.Refresh();
        ...
        private async Task CargarListaVuelos()
        {
            try
            {
                ...
                {
                    string url = $"{API_BASE}/vuelo?page=1&limit=100";
                    Console.WriteLine($"Cargando vuelos desde: {url}");
                    ...
                    if (parsed is Newtonsoft.Json.Linq.JObject jObject)
                    {
                        dataArray = jObject["data"] as Newtonsoft.Json.Linq.JArray;
                    }
                    else if (parsed is Newtonsoft.Json.Linq.JArray jArray)
                    {
                        dataArray = jArray;
                    }
                    ...
                    Console.WriteLine($"=== RESPUESTA COMPLETA DE /vuelo ===");
                    Console.WriteLine(response);

                    if (dataArray.Count > 0)
                    {
                        // Loguear la estructura completa del primer vuelo
                        var primerVuelo = dataArray[0];
                        Console.WriteLine($"\n=== ESTRUCTURA COMPLETA DEL PRIMER VUELO ===");
                        Console.WriteLine(primerVuelo.ToString());

                        if (primerVuelo is Newtonsoft.Json.Linq.JObject jObj)
                        {
                            Console.WriteLine($"\n=== CAMPOS DISPONIBLES ===");
                            foreach (var prop in jObj.Properties())
                            {
                                Console.WriteLine($"Campo: {prop.Name} = {prop.Value}");
                            }
                        }
                    }
                    ...
                    {
                        // Debug: mostrar estructura de este item
                        if (item is Newtonsoft.Json.Linq.JObject debugObj)
                        {
                            Console.WriteLine($"\n>>> Item properties:");
                            foreach (var p in debugObj.Properties())
                                Console.WriteLine($"    {p.Name}: {p.Value}");
                        }

                        // Intentar múltiples nombres de campo posibles
                        string idVuelo = item["id"]?.ToString() ??
                                        item["_id"]?.ToString() ??
                                        item["ID"]?.ToString() ??
                                        item["vuelo_id"]?.ToString();

                        string nametag = item["nametag"]?.ToString() ??
                                        item["Nametag"]?.ToString();

                        string datetime = item["datetime"]?.ToString() ??
                                         item["Datetime"]?.ToString();

                        Console.WriteLine($"Extracted - ID='{idVuelo}', Nametag='{nametag}'");

                        if (string.IsNullOrEmpty(idVuelo))
                        {
                            Console.WriteLine($"⚠️  ERROR: No se encontró campo ID para: {nametag}");
                            continue;
                        }
                        ...
                        Console.WriteLine($"✓ Vuelo agregado: ID={idVuelo}, Nametag={nametag}, Datetime={datetime}");
                    }
        ...
        private async Task CargarWaypointsBD(string vueloId)
        {
            try
            {
                ...
                using (var client = new System.Net.Http.HttpClient())
                {
                    ...
                    Console.WriteLine($"VueloId codificado: {encodedId}");
                    ...
                    if (!response.IsSuccessStatusCode)
                    {
                        ...
                        MessageBox.Show($"Error del servidor ({response.StatusCode}): {errorContent}\n\nURL enviada: {url}", "Error de API");
                        return;
                    }
                    ...
                    else if (parsed is Newtonsoft.Json.Linq.JObject jObject)
                    {
                        ...
                        var dataField = jObject["data"] as Newtonsoft.Json.Linq.JArray;
                        if (dataField != null)
                        {
                    ...
                    {
                        Console.WriteLine($"No hay instrucciones para este vuelo. Respuesta: {responseContent}");
                    ...
                    int count = 0;
                    foreach (var instruccion in ordenadas)
                    {
                        var punto = instruccion["Punto"];
                        if (punto == null) 
                        {
                            Console.WriteLine($"Instrucción sin campo Punto en trail {instruccion["trail"]}");
                            continue;
                        }

                        // Parsear Latitud
                        if (!double.TryParse(punto["Latitud"]?.ToString(),
                                System.Globalization.NumberStyles.Float,
                                System.Globalization.CultureInfo.InvariantCulture, out double lat)) 
                        {
                            Console.WriteLine($"No se pudo parsear Latitud en trail {instruccion["trail"]}: {punto["Latitud"]}");
                            continue;
                        }

                        // Parsear Longitud
                        if (!double.TryParse(punto["Longitud"]?.ToString(),
                                System.Globalization.NumberStyles.Float,
                                System.Globalization.CultureInfo.InvariantCulture, out double lon)) 
                        {
                            Console.WriteLine($"No se pudo parsear Longitud en trail {instruccion["trail"]}: {punto["Longitud"]}");
                            continue;
                        }

                        count++;
                        var wp = new PointLatLng(lat, lon);
                        bdWaypoints.Add(wp);
                        bdWaypointsListBox.Items.Add($"WP {count}: ({lat:F6}, {lon:F6})");

                        var marker = new WaypointMarker(wp, count);
                        bdWaypointsOverlay.Markers.Add(marker);

                        Console.WriteLine($"✓ Waypoint {count} (trail {instruccion["trail"]}): Lat={lat}, Lon={lon}");
                    }
            ...
            catch (System.Net.Http.HttpRequestException ex)
            {
                ...
                Console.WriteLine($"Error HTTP: {ex}");
            }
            catch (Exception ex)
            {
