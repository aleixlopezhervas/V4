...
namespace Formulario
{
    ...
    public partial class App : Form
    {
            ...
            if (dLat < WAYPOINT_ARRIVAL_THRESHOLD && dLon < WAYPOINT_ARRIVAL_THRESHOLD)
            {
                ...
                if (currentWaypointIndex < waypoints.Count)
                {
                    ...
                    ThreadPool.QueueUserWorkItem(_ =>
                    {
                        try
                        {
                            if (currentWaypoint.Heading != 0 && currentWaypoint.Directriz != "ChangeHeading")
                            {
                                dron.EscribirParametros(new List<(string, float)> { ("WP_YAW_BEHAVIOR", 0f) });
                                Thread.Sleep(300);
                                dron.CambiarHeading(currentWaypoint.Heading, bloquear: true);
                            }
                            else
                            {
                                dron.EscribirParametros(new List<(string, float)> { ("WP_YAW_BEHAVIOR", 1f) });
        ...
        {
            // Ajustar mapa para dar espacio abajo
            var bdMapPanel = this.Controls.Find("bdMapPanel", true).FirstOrDefault() as Panel;
            if (bdMapPanel != null) bdMapPanel.Size = new Size(760, 270);

            // Ajustar label
            var lblWaypoints = this.Controls.Find("bdWaypointsLabel", true).FirstOrDefault() as Label;
            if (lblWaypoints != null) 
            {
                lblWaypoints.Location = new Point(260, 280);
                lblWaypoints.Size = new Size(760, 20);
            }

            // Ajustar listbox
            var lstWaypoints = this.Controls.Find("bdWaypointsListBox", true).FirstOrDefault() as ListBox;
            if (lstWaypoints != null) 
            {
                lstWaypoints.Location = new Point(260, 300);
                lstWaypoints.Size = new Size(760, 60);
            }

            // Panel contenedor principal de vídeos (debajo de listbox)
            ...
            // ---- CAJA IZQUIERDA: GRABANDO ----
            Panel boxIzquierda = new Panel
            {
                Location = new Point(0, 0),
                Size = new Size(375, 170),
            ...
            boxIzquierda.Controls.Add(bdVideoPreview);

            bdRecordingStatusLabel = new Label
            {
                Location = new Point(2, 150),
            ...
            boxIzquierda.Controls.Add(bdRecordingStatusLabel);
            ...
            boxIzquierda.Controls.Add(bdOpenFolderBtn);

            bdVideoPanel.Controls.Add(boxIzquierda);

            // ---- CAJA DERECHA: REPRODUCCIÓN ----
            Panel boxDerecha = new Panel
            {
            ...
            bdVideoPlaybackBrowser = new WebBrowser
            {
                Location = new Point(0, 0),
                Size = new Size(375, 170),
            ...
            boxDerecha.Controls.Add(bdVideoPlaybackBrowser);

            bdVideoPanel.Controls.Add(boxDerecha);

            // Añadir panel a la pestaña
            ...
            // Inicializar timer para preview en vivo (caja izquierda)
